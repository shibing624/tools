...
def predict():
    example_sentence = "我跟我朋唷打算去法国玩儿。"

    tokenizer = AutoTokenizer.from_pretrained('./byt5-base-zh-correction')

    model_inputs = tokenizer(example_sentence, max_length=128, truncation=True, return_tensors="pt")

    model = T5ForConditionalGeneration.from_pretrained('./byt5-base-zh-correction')
...
def train():
    args_dict = {
        ...
        "max_len": 128,
        "output_dir": './byt5-base-zh-correction',
        ...
        "fp16": False,
        "use_cache": False,
        "max_steps": 10  # default 5000
    }
    parser = HfArgumentParser(
        (ModelArguments, DataTrainingArguments, TrainingArguments))
    model_args, data_args, training_args = parser.parse_dict(args_dict)
    set_seed(training_args.seed)
    dataset = load_dataset('csv', data_files={'train': ['data/zh_test_100.csv'], 'test': 'data/zh_test_100.csv'})
    ...
    # r = tokenizer.encode('hi world.', max_length=128, truncation=True)
    # print(r)
    ...
    dataset = dataset.map(
        lambda x: {'ocr_text': x['ocr_text'], 'text': x['text']},
        batched=True)
    ...
    train_dataset = train_dataset.select(range(90))
    valid_dataset = valid_dataset.select(range(90))
    ...
    def prep_dataset(tokenizer, dataset, max_len):
        def convert_to_features(example_batch):
            ...
            print(example_batch['ocr_text'])
            print('input_encodings:', input_encodings)
