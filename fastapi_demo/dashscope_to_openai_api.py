...
# Response转换
def convert_response_to_choice(chat_response_choices) -> List[ChatCompletionResponseChoice]:
    index = 0
    choices: List[ChatCompletionResponseChoice] = []
    for i in chat_response_choices:
        choices.append(
...
async def predict(
    ...
    for new_response in response_generator:
        if new_response.status_code == HTTPStatus.OK:
            if len(new_response.output.text) == current_length:
                continue

            new_text = new_response.output.text[current_length:]
            current_length = len(new_response.output.text)
        else:
            continue
        choice_data = ChatCompletionResponseStreamChoice(
